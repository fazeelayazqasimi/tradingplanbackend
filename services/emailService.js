...
const buildTemplate = (title, bodyContent) => {
  return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
            ...
            <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
              <tr>
                <td style="background-color:#3b82f6;padding:24px 30px;text-align:center;">
                  <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:600;">Trading Institute</h1>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td style="background-color:#f9fafb;padding:20px 30px;text-align:center;border-top:1px solid #e5e7eb;">
                  <p style="margin:0 0 6px 0;font-size:13px;color:#6b7280;">Trading Institute Management System</p>
...
const sendEmail = async ({ to, subject, html }) => {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"Trading Institute" <${process.env.EMAIL_USER}>`,
      to,
      subject,
      html
    });
...
const sendWelcomeEmail = async (user) => {
  const html = buildTemplate('Welcome to Trading Institute', `
    ...
    <p style="margin:0 0 12px 0;font-size:15px;color:#374151;line-height:1.6;">
      Your account has been created successfully. You now have access to our trading education platform.
    </p>
    <p style="margin:0 0 12px 0;font-size:15px;color:#374151;line-height:1.6;">
  ...
  return sendEmail({
    to: user.email,
    subject: 'Welcome to Trading Institute',
    html
  });
};
...
const sendPasswordResetEmail = async (user, resetUrl) => {
  ...
  return sendEmail({
    to: user.email,
    subject: 'Password Reset Request - Trading Institute',
    html
  });
};
...
const sendPaymentReceivedEmail = async (user, amount) => {
  ...
  return sendEmail({
    to: user.email,
    subject: 'Payment Received - Trading Institute',
    html
  });
};
...
const sendAccountApprovedEmail = async (user) => {
  ...
  return sendEmail({
    to: user.email,
    subject: 'Account Approved - Trading Institute',
    html
  });
};
...
const sendSubscriptionReminderEmail = async (user, daysLeft) => {
  ...
  return sendEmail({
    to: user.email,
    subject: `Your Subscription Expires in ${daysLeft} Day${daysLeft !== 1 ? 's' : ''} - Trading Institute`,
    html
  });
};
...
const sendCommissionReceivedEmail = async (user, amount) => {
  ...
  return sendEmail({
    to: user.email,
    subject: `Commission of $${Number(amount).toFixed(2)} Received - Trading Institute`,
    html
  });
};
...
const sendWithdrawalApprovedEmail = async (user, amount) => {
  ...
  return sendEmail({
    to: user.email,
    subject: 'Withdrawal Approved - Trading Institute',
    html
  });
};
...
const sendRankPromotionEmail = async (user, newRank) => {
  ...
  return sendEmail({
    to: user.email,
    subject: `You've Been Promoted to ${newRank.name}! - Trading Institute`,
    html
  });
};
...
const sendSignalPublishedEmail = async (users, signal) => {
  ...
  const results = await Promise.allSettled(
    userArray.map((user) => sendEmail({
      to: user.email,
      subject: `New Signal: ${signal.action} ${signal.symbol} at ${signal.openPrice} - Trading Institute`,
      html
    }))
  );
...
const sendAnnouncementEmail = async (users, announcement) => {
  ...
  const results = await Promise.allSettled(
    userArray.map((user) => sendEmail({
      to: user.email,
      subject: `${announcement.title} - Trading Institute`,
      html
    }))
  );
...
const sendReferralSignupEmail = async (user, referredUser) => {
  ...
  `);
  return sendEmail({ to: user.email, subject: 'New Referral Signup - Trading Institute', html });
};
...
const sendCourseEnrollmentPendingEmail = async (user, course) => {
  ...
  `);
  return sendEmail({ to: user.email, subject: `Enrollment Pending - ${course.title} - Trading Institute`, html });
};
...
const sendAccountDeactivatedEmail = async (user) => {
  ...
  `);
  return sendEmail({ to: user.email, subject: 'Account Deactivated - Trading Institute', html });
};
...
const sendVerificationEmail = async (user, verificationUrl) => {
  const html = buildTemplate('Verify Your Email', `
    ...
    <p style="margin:0 0 12px 0;font-size:15px;color:#374151;line-height:1.6;">
      Hi ${user.firstName}, thank you for registering at Trading Institute.
    </p>
    <p style="margin:0 0 12px 0;font-size:15px;color:#374151;line-height:1.6;">
  ...
  return sendEmail({
    to: user.email,
    subject: 'Verify Your Email - Trading Institute',
    html
  });
};
...
const sendOTPEmail = async (email, otp) => {
  ...
  return sendEmail({
    to: email,
    subject: 'Your OTP Code - Trading Institute',
    html
  });
};
